...
def amplitude_plot(measure, Model, data_label, data_range=(9150, 9550), plot_range=None, kind='filter', burn_in=1000):
     ...
     #axs[2].plot(range(plot_init, plot_final), results.local_level[(plot_init - init + burn_in):(plot_final - init + burn_in)])
     #axs[2].set_ylabel('Velocity')
     #axs[2].set_title('Local Level')
     
     fig.tight_layout()
     plt.show()

ModelVel = get_models()[0]
amplitude_plot('xvelocity', get_models()[0], 'Velocity', data_range=(600, 1050), plot_range=(750, 1005), kind='forecast')
#amplitude_plot('xvelocity', get_models()[0], 'Velocity', data_range=(1, 1050), plot_range=(500, 750), kind='forecast')
#amplitude_plot('xvelocity', get_models()[0], 'Velocity', data_range=(1, 1050), plot_range=(50, 260), kind='forecast')
